...
for my $test (@tests) {
	...
	if ($ip6 && !cPanel::IO::Socket::SSL->can_ipv6) {
	    skip "no IPv6 support - $diag",1;
	}
	if ($idn && ! eval { cPanel::IO::Socket::SSL::idn_to_ascii("fo") }) {
	...
	#diag($diag);
	my $match = cPanel::IO::Socket::SSL::verify_hostname_of_cert($hostname,$cert,'www')||0;
