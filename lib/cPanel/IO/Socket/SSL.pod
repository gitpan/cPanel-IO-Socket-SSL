...
cPanel::IO::Socket::SSL - SSL sockets with IO::Socket interface
...
=head1 SYNOPSIS
    ...
    # simple client
    my $cl = cPanel::IO::Socket::SSL->new('www.google.com:443');
    print $cl "GET / HTTP/1.0\r\n\r\n";
    print <$cl>;

    # simple server
    my $srv = cPanel::IO::Socket::SSL->new(
...
cPanel::IO::Socket::SSL makes using SSL/TLS much easier by wrapping the necessary
...
=item * L<cPanel::IO::Socket::SSL::Intercept> - Doing Man-In-The-Middle with SSL

=item * L<cPanel::IO::Socket::SSL::Utils> - Useful functions for certificates etc
...
A basic SSL client is simple:
    ...
    my $client = cPanel::IO::Socket::SSL->new('www.example.com:443')
...
In the default settings cPanel::IO::Socket::SSL will use a safer cipher set and SSL
...
A more complete example for a simple HTTP client:
	...
	cPanel::IO::Socket::SSL::default_ca(), # either explicitly
...
higher and Net::SSLeay at least 1.59):
    ...
    my $client = cPanel::IO::Socket::SSL->new($dst);
    ...
    my $ocsp_cache = cPanel::IO::Socket::SSL::OCSP_Cache->new;
    my $client = cPanel::IO::Socket::SSL->new(
	PeerAddr => $dst,
...
This is a list of typical errors seen with the use of cPanel::IO::Socket::SSL:
...
Because cPanel::IO::Socket::SSL is now able to provide a usable CA store on most
plattforms (UNIX, Mac OSX and Windows) it is better to use the defaults provided
by cPanel::IO::Socket::SSL.
...
cPanel::IO::Socket::SSL tries to set these values to reasonable secure values, which
...
cPanel::IO::Socket::SSL sets and which get updated from time to time to
...
Some modules use cPanel::IO::Socket::SSL, but don't make the SSL settings available
...
the debug settings of cPanel::IO::Socket::SSL and L<Net::SSLeay>, which can both be
set with C<$cPanel::IO::Socket::SSL::DEBUG>.
...
1 - Print out errors from cPanel::IO::Socket::SSL and ciphers from Net::SSLeay.

=item *

2 - Print also information about call flow from cPanel::IO::Socket::SSL and progress
information from Net::SSLeay.

=item *

3 - Print also some data dumps from cPanel::IO::Socket::SSL and from Net::SSLeay.
...
cPanel::IO::Socket::SSL uses OpenSSL with the help of the L<Net::SSLeay> library. It
...
on the new socket inside C<cPanel::IO::Socket::SSL::accept> will be done in a blocking
...
TCP socket in a non-blocking way with C<start_SSL> and C<accept_SSL>.
    ...
    my $cl = cPanel::IO::Socket::SSL->new($dst);
    $cl->blocking(0);
...
higher in cPanel::IO::Socket::SSL.
...
To check for support you might call C<cPanel::IO::Socket::SSL->can_client_sni()>.

On the server side earlier versions of OpenSSL are supported, but only together
with L<Net::SSLeay> version >= 1.50.
To check for support you might call C<cPanel::IO::Socket::SSL->can_server_sni()>.
...
to upgrade and stop_SSL to downgrade:
    ...
    cPanel::IO::Socket::SSL->start_SSL($sock,%sslargs) or die $SSL_ERROR;
    ... now $sock is a cPanel::IO::Socket::SSL object ...
...
stop_SSL to talk plain again:
    ...
    ... exchange data with SSL on $sock until stoptls command ...
    $sock->stop_SSL or die $SSL_ERROR;
    ... $sock is still a cPanel::IO::Socket::SSL object ...
...
cPanel::IO::Socket::SSL behaves similar to other L<IO::Socket> modules and thus could
...
cPanel::IO::Socket::SSL inherits from another IO::Socket module.
...
as C<Domain> of the socket or globally enforce IPv4 by loading cPanel::IO::Socket::SSL
with the option 'inet4'.

cPanel::IO::Socket::SSL will provide all of the methods of its super class, but
...
Creates a new cPanel::IO::Socket::SSL object.  You may use all the friendly options
...
L<cPanel::IO::Socket::SSL::Utils>) as a list with C<SSL_ca>. These will be added to the
...
modules using cPanel::IO::Socket::SSL when CA is not exlicitly given).
...
If this is true, it forces cPanel::IO::Socket::SSL to use a certificate and key, even if
...
C<cPanel::IO::Socket::SSL->can_ecdh>.
...
L<cPanel::IO::Socket::SSL::PublicSuffix>, you can create another object with
...
with C<< cPanel::IO::Socket::SSL->new >>, if an IO::Socket::INET object is upgraded
...
C<<cPanel::IO::Socket::SSL::OCSP_Cache->new([size]) >> or implement your own cache,
...
cPanel::IO::Socket::SSL, then you can reuse the SSL context of that instance by passing
it as the value for the SSL_reuse_ctx parameter.  You may also create a
new instance of the cPanel::IO::Socket::SSL::SSL_Context class, using any context
...
invalid.  Note that, contrary to versions of cPanel::IO::Socket::SSL below v0.90, a
...
C<< cPanel::IO::Socket::SSL::Session_Cache->new( cachesize ) >>.
...
openssl-1.0.1+.
To check support you might call C<cPanel::IO::Socket::SSL->can_npn()>.
...
Returns the string form of the cipher that the cPanel::IO::Socket::SSL object is using.
...
constants are exported from cPanel::IO::Socket::SSL).
...
L<cPanel::IO::Socket::SSL::Utils> to inspect each of the certificates.
...
real object to perform this method on, call cPanel::IO::Socket::SSL::errstr() instead.
...
=item B<< cPanel::IO::Socket::SSL->start_SSL($socket, ... ) >>

This will convert a glob reference or a socket that you provide to an
cPanel::IO::Socket::SSL object.	 You may also pass parameters to specify context or
...
cPanel::IO::Socket::SSL->start_SSL($socket, SSL_server => 1).  If you have a class that
inherits from cPanel::IO::Socket::SSL and you want the $socket to be blessed into your
...
socket to cPanel::IO::Socket::SSL and call accept_SSL or connect_SSL and the upgraded
...
more requests are outstanding. cPanel::IO::Socket::SSL will combine multiple OCSP
...
=item B<< cPanel::IO::Socket::SSL->new_from_fd($fd, [mode], %sslargs) >>
...
=item B<cPanel::IO::Socket::SSL::default_ca([ path|dir| SSL_ca_file => ..., SSL_ca_path => ... ])>
...
=item B<cPanel::IO::Socket::SSL::set_default_context(...)>

You may use this to make cPanel::IO::Socket::SSL automatically re-use a given context
(unless specifically overridden in a call to new()).
It accepts one argument, which should be either an cPanel::IO::Socket::SSL object or an
cPanel::IO::Socket::SSL::SSL_Context object.
...
=item B<cPanel::IO::Socket::SSL::set_default_session_cache(...)>

You may use this to make cPanel::IO::Socket::SSL automatically re-use a given session
cache (unless specifically overridden in a call to new()).
It accepts one argument, which should be an cPanel::IO::Socket::SSL::Session_Cache
object or similar (e.g something which implements get_session and add_session
like cPanel::IO::Socket::SSL::Session_Cache does).
...
=item B<cPanel::IO::Socket::SSL::set_defaults(%args)>
...
=item B<cPanel::IO::Socket::SSL::set_client_defaults(%args)>

Similar to C<set_defaults>, but only sets the defaults for client mode.

=item B<cPanel::IO::Socket::SSL::set_server_defaults(%args)>

Similar to C<set_defaults>, but only sets the defaults for server mode.

=item B<cPanel::IO::Socket::SSL::set_args_filter_hack(\&code|'use_defaults')>
...
Example:
    ...
    cPanel::IO::Socket::SSL::set_args_filter_hack( sub {
	my ($is_server,$args) = @_;
	if ( ! $is_server ) {
...
cPanel::IO::Socket::SSL will emit a large CROAK() if you are silly enough to use them:
...
(such as that used by cPanel::IO::Socket::SSL) and so may send unencrypted data over the
...
use cPanel::IO::Socket::SSL->start_SSL() instead
...
If you use cPanel::IO::Socket::SSL together with threads you should load it (e.g. use or
...
Creating an cPanel::IO::Socket::SSL object in one thread and closing it in another
thread will not work.

cPanel::IO::Socket::SSL does not work together with Storable::fd_retrieve/fd_store.
...
Many thanks to all who added patches or reported bugs or helped cPanel::IO::Socket::SSL
...
Marko Asplund, <marko.asplund at kronodoc.fi>, was the original author of cPanel::IO::Socket::SSL.
