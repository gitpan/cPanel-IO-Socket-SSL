...
package cPanel::IO::Socket::SSL::Intercept;
use strict;
use warnings;
use Carp 'croak';
use cPanel::IO::Socket::SSL::Utils;
use Net::SSLeay;
...
cPanel::IO::Socket::SSL::Intercept -- SSL interception (man in the middle)
...
=head1 SYNOPSIS
	...
	cPanel::IO::Socket::SSL->start_SSL($client,
	    SSL_server => 1,
	    SSL_cert => $cert,
	    SSL_key => $key
...
This a the step where cPanel::IO::Socket::SSL::Intercept helps.
...
cPanel::IO::Socket::SSL::Intercept helps creating the cloned certificate with the
following methods:

=over 4

=item B<< $mitm = cPanel::IO::Socket::SSL::Intercept->new(%args) >>
...
=item B<< $mitm = cPanel::IO::Socket::SSL::Intercept->unserialize($string) >>
