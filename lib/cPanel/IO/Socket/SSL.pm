...
#
# cPanel::IO::Socket::SSL:
# provide a compilable interface to SSL connections similar to IO::Socket modules
#
# Current Code Shepherds: cPanel Inc <cpan@cpanel.net> (Reini Urban, Todd Rinaldo),
...
use IO::Socket;
use Net::SSLeay 1.46;
use cPanel::IO::Socket::SSL::PublicSuffix;
use Exporter ();
...
# needs to be initialized at run-time, not compile-time. perlcc code cannot
...
INIT {
    # Do run-time Net::SSLeay initialization
...
BEGIN {
    ...
    *idn_to_ascii = \&cPanel::IO::Socket::SSL::PublicSuffix::idn_to_ascii;
    *idn_to_unicode = \&cPanel::IO::Socket::SSL::PublicSuffix::idn_to_unicode;
}
...
sub configure {
    ...
    # otherwise cPanel::IO::Socket::SSL->new might return undef if the
...
sub configure_SSL {
    ...
    if ($ctx) {
	if ($ctx->isa('cPanel::IO::Socket::SSL::SSL_Context') and
	    $ctx->{context}) {
	    # valid context
    ...
    $ctx ||= cPanel::IO::Socket::SSL::SSL_Context->new($arg_hash);
...
sub connect_SSL {
    ...
    tie *{$self}, "cPanel::IO::Socket::SSL::SSL_HANDLE", $self;
...
sub accept {
    my $self = shift || return _invalid_object();
    my $class = shift || 'cPanel::IO::Socket::SSL';
...
sub accept_SSL {
    ...
    tie *{$socket}, "cPanel::IO::Socket::SSL::SSL_HANDLE", $socket;
...
####### cPanel::IO::Socket::SSL specific functions #######
# _get_ssl_object is for internal use ONLY!
sub _get_ssl_object {
    my $self = shift;
    return ${*$self}{'_SSL_object'} ||
	cPanel::IO::Socket::SSL->_internal_error("Undefined SSL object");
}
...
sub _invalid_object {
    return cPanel::IO::Socket::SSL->_internal_error("Undefined cPanel::IO::Socket::SSL object");
}
...

{
	...
	my $check_name = sub {
	    ...
	    if ( $identity =~ $pattern ) {
		$publicsuffix = cPanel::IO::Socket::SSL::PublicSuffix->default
...
if ($can_ocsp) {
    ...
    *ocsp_resolver = sub {
	...
	return cPanel::IO::Socket::SSL::OCSP_Resolver->new(
	    $ssl,
	    $ctx->{ocsp_cache} ||= cPanel::IO::Socket::SSL::OCSP_Cache->new,
...
sub socket_to_SSL { cPanel::IO::Socket::SSL->start_SSL(@_); }
sub socketToSSL { cPanel::IO::Socket::SSL->start_SSL(@_); }
...
sub ungetc { croak("Use of ungetc() not implemented in cPanel::IO::Socket::SSL") }
sub send   { croak("Use of send() not implemented in cPanel::IO::Socket::SSL; use print/printf/syswrite instead") }
sub recv   { croak("Use of recv() not implemented in cPanel::IO::Socket::SSL; use read/sysread instead") }

package cPanel::IO::Socket::SSL::SSL_HANDLE;
use strict;
use Errno 'EBADF';
*weaken = *cPanel::IO::Socket::SSL::weaken;
...
package cPanel::IO::Socket::SSL::SSL_Context;
use Carp;
use strict;

my %CTX_CREATED_IN_THIS_THREAD;
*DEBUG = *cPanel::IO::Socket::SSL::DEBUG;
...
sub new {
	...
	$is_server ? cPanel::IO::Socket::SSL::DEFAULT_SSL_SERVER_ARGS() : %DEFAULT_SSL_CLIENT_ARGS,
	%$arg_hash
    );
	...
	$arg_hash->{SSL_verify_callback} = sub {
	    ...
	    if ( ! $host ) {
		if ( $vcn_scheme ) {
		    cPanel::IO::Socket::SSL->_internal_error(
	    ...
	    # verify name
	    my $rv = cPanel::IO::Socket::SSL::verify_hostname_of_cert(
...
WARN
	...
	'CTX_new'
    ) or return cPanel::IO::Socket::SSL->_internal_error("SSL Version $ver not supported");
    my $ctx = $ctx_new_sub->() or return
	cPanel::IO::Socket::SSL->error("SSL Context init failed");
	...
	return cPanel::IO::Socket::SSL->_internal_error("NPN not supported in Net::SSLeay")
	    if ! $can_npn;
	...
	if ( $arg_hash->{SSL_ca} ) {
		...
		Net::SSLeay::X509_STORE_add_cert($store,$_) or
		    return cPanel::IO::Socket::SSL->error(
    ...
    } elsif ( my %ca = cPanel::IO::Socket::SSL::default_ca()) {
	...
	if ($arg_hash->{'SSL_crl_file'}) {
		...
		return cPanel::IO::Socket::SSL->error("Invalid certificate revocation list");
	    }
	}
    }
	...
	for my $sni (values %sni) {
		...
		Net::SSLeay::CTX_use_certificate( $snictx,$cert )
		    || return cPanel::IO::Socket::SSL->error("Failed to use Certificate");
		foreach my $ca (@x509) {
		...
		$havecert or return
		    cPanel::IO::Socket::SSL->error("Failed to use certificate file");
	    }
		...
		Net::SSLeay::CTX_use_PrivateKey($snictx, $pkey)
		    || return cPanel::IO::Socket::SSL->error("Failed to use Private Key");
		$havekey = 'MEM';
	    ...
	    $havekey or return
		cPanel::IO::Socket::SSL->error("Failed to use private key");
	}

	if ( keys %sni > 1 or ! exists $sni{''} ) {
	    # we definitely want SNI support
	    $can_server_sni or return cPanel::IO::Socket::SSL->_internal_error(
	...
	if ( my $f = $arg_hash->{SSL_dh_file} ) {
		...
		|| return cPanel::IO::Socket::SSL->error( "Failed to open DH file $f" );
	    my $dh = Net::SSLeay::PEM_read_bio_DHparams($bio);
	    Net::SSLeay::BIO_free($bio);
	    $dh || return cPanel::IO::Socket::SSL->error( "Failed to read PEM for DH from $f - wrong format?" );
	    my $rv = Net::SSLeay::CTX_set_tmp_dh( $ctx,$dh );
	    Net::SSLeay::DH_free( $dh );
	    $rv || return cPanel::IO::Socket::SSL->error( "Failed to set DH from $f" );
	} elsif ( my $dh = $arg_hash->{SSL_dh} ) {
	    # binary, e.g. DH*
	    Net::SSLeay::CTX_set_tmp_dh( $ctx,$dh )
		|| return cPanel::IO::Socket::SSL->error( "Failed to set DH from SSL_dh" );
	}

	if ( my $curve = $arg_hash->{SSL_ecdh_curve} ) {
	    return cPanel::IO::Socket::SSL->_internal_error(
	...
	for( ref($fp) ? @$fp : $fp) {
	    ...
	    return cPanel::IO::Socket::SSL->_internal_error("invalid fingerprint '$_'")
		if ! $algo;
	    $algo = lc($algo);
	...
	Net::SSLeay::CTX_set_cipher_list($ctx, $cl )
	    || return cPanel::IO::Socket::SSL->error("Failed to set SSL cipher list");
    }
	...
	$self->{verify_mode} ? cPanel::IO::Socket::SSL::SSL_OCSP_TRY_STAPLE() :
	0;
	...
	$self->{session_cache} = cPanel::IO::Socket::SSL::Session_Cache->new( $size );
    }
...
package cPanel::IO::Socket::SSL::Session_Cache;
use strict;
...
package cPanel::IO::Socket::SSL::OCSP_Cache;
...
package cPanel::IO::Socket::SSL::OCSP_Resolver;
*DEBUG = *cPanel::IO::Socket::SSL::DEBUG;
