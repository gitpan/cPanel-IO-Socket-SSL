#
# a test server for testing cPanel::IO::Socket::SSL-class's behavior
...
if($ARGV[0] eq "DEBUG") { $cPanel::IO::Socket::SSL::DEBUG = 1; }
...
unless (-d "certs") {
	...
	die "Please run this example from the cPanel::IO::Socket::SSL distribution directory!\n";
    }
}

if(!($sock = cPanel::IO::Socket::SSL->new( Listen => 5,
				   LocalAddr => 'localhost',
				   LocalPort => 9000,
				   Proto     => 'tcp',
				   Reuse     => 1,
    ...
    warn "unable to create socket: ", &cPanel::IO::Socket::SSL::errstr, "\n";
    exit(0);
}
...
while (1) {
  ...
  while(($s = $sock->accept())) {
      ...
      if( ref($sock) eq "cPanel::IO::Socket::SSL") {
