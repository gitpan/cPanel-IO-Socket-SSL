...
use warnings;
use Socket;
use cPanel::IO::Socket::SSL 1.984;
use cPanel::IO::Socket::SSL::Utils;
use Getopt::Long qw(:config posix_default bundling);


my $can_ocsp = cPanel::IO::Socket::SSL->can_ocsp;
my $ocsp_cache = $can_ocsp && cPanel::IO::Socket::SSL::OCSP_Cache->new;
...
GetOptions(
    'h|help' => sub { usage() },
    'v|verbose:1' => \$verbose,
    'd|debug:1' => \$cPanel::IO::Socket::SSL::DEBUG,
    'T|timeout=i' => \$timeout,
    'CApath=s' => \$capath,
...
die "need Net::SSLeay>=1.58 for showing chain" if $show_chain
    && ! defined &cPanel::IO::Socket::SSL::peer_certificates;
...
Options:
  ...
  -d|--debug level       - cPanel::IO::Socket::SSL/Net::SSLeay debugging
...
my $ioclass = cPanel::IO::Socket::SSL->can_ipv6 || 'IO::Socket::INET';
for my $test (@tests) {
	...
	my $cl = &$tcp_connect;
	if ( cPanel::IO::Socket::SSL->start_SSL($cl, %conf,
	    SSL_verify_mode => 0,
	    SSL_hostname => $name,
	)) {
